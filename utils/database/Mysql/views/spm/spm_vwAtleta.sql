...
-- Script date 22/01/2018 11:05:04
...
SELECT
  ...
  pipeMask(tbnaturalperson.ssn_cpf,'###.###.###-##') AS cpf_fmt,
  tbnaturalperson.rg,
  tbnaturalperson.rgoexp,
  tbnaturalperson.dtbirth,
  DATE_FORMAT(tbnaturalperson.dtbirth, '%d/%m/%Y') AS dtbirth_fmt,
  ...
  spm_tbdepartamento.nome departamento,
  (SELECT spm_tbapelido.nome FROM spm_tbapelido WHERE spm_tbapelido.idperson = tbperson.idperson) AS apelido
FROM tbperson
