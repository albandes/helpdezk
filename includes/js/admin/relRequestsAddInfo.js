$(function(){
	...
	$(document.getElementById("formSearch")).submit(function(){
    	...
    	$.ajax({
			...
			data: $(this).serialize(),
			dataType : "json",
			url:'relRequestsAddInfo/table_json/',
			success: function(data){
				if(data.length == 0)
					$table.append('<tr class="noRegister"><td colspan="4">'+aLang['No_result'].replace (/\"/g, "")+'</td></tr>');
                else{
                    $.each(data, function(key, val) {
                        $table.append('<tr><td>'+ val.code_request +'</td><td>'+ val.person +'</td><td>'+ val.date +'</td><td>'+ val.addinfo +'</td></tr>');
                    });	
                }
			},
			beforeSend: function(){
	...
	$(document.getElementById('modalSalvar')).find('form').submit(function(){
        ...
        window.open(path + "/app/reports/relRequestsAddInfo.php?outputtype=" + outputtype + "&delimiter=" + txtSeparator + "&fromdate=" + fromdate + "&todate=" + todate + "&addInfo=" + addInfo + "&headertab=" + headerTab, "_blank");
