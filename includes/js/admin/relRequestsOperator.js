$(function(){
	...
	$(document.getElementById("formSearch")).submit(function(){
    	var $boxRetorno = $(document.getElementById('boxRetorno')),
    		...
    		$showDesc	= $(document.getElementById("showDesc")).is(":checked");
    	...
    	$.ajax({
			...
			data: $(this).serialize(),
			dataType : "json",
			url:'relRequestsOperator/table_json/',
	                	...
	                	$.each(val.sol, function(key, sol) {
	                		...
	                		if(val.length == 0)
		                        ...
		                        $table.append('<tr><td>'+ sol.code +'</td><td>'+ sol.name +'</td><td>'+ sol.company +'</td><td>'+ sol.subject +'</td><td>'+ sol.entry_date +'</td><td>'+ sol.priority +'</td><td>'+ sol.status +'</td><td>'+ valRepass+'</td></tr>');
			                  	...
			                  	if($showDesc)
			                  		$table.append('<tr><td colspan="8"><strong>'+aLang['Description'].replace (/\"/g, "")+':</strong> '+ sol.description +'</td></tr>');
			                }
			                total++;
	                	})	
	...
	$(document.getElementById('modalSalvar')).find('form').submit(function(){
    	...
    	if($(document.getElementById("showDesc")).is(":checked"))
    		showDescValue = "Y";
    	else
    		showDescValue = "N";
        ...
        window.open(path + "/app/reports/relRequestsOperator.php?outputtype=" + outputtype + "&delimiter=" + txtSeparator + "&operator=" +cmbPerson+ "&status=" +status+ "&cmbCompany=" +cmbCompany+ "&txtPriority=" +txtPriority+ "&txtowner=" +txtowner+ "&cmbType=" +cmbType+ "&cmbItem=" +cmbItem+ "&cmbService=" +cmbService+ "&fromdate=" + fromdate + "&todate=" + todate + "&showDesc=" + showDescValue + "&headertab=" + headerTab, "_blank");
