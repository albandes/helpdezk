$(function(){
	...
	$(document.getElementById("formSearch")).submit(function(){
    	...
    	$.ajax({
			...
			data: $(this).serialize(),
			dataType : "json",
			url:'relRequestMonth/table_json/',
			success: function(data){
				...
				if(data.length == 0)
                    ...
                    $.each(data, function(key, val) {                    		
                        $table.append('<tr><td>'+ val.month_txt +'</td><td>'+ val.total +'</td></tr>');
                    });	
                }
			},
			beforeSend: function(){
	...
	$(document.getElementById('modalSalvar')).find('form').submit(function(){
        ...
        window.open(path + "/app/reports/relReportMonth.php?outputtype=" + outputtype + "&delimiter=" + txtSeparator + "&cmbCompany=" +cmbCompany+ "&cmbPerson=" +cmbPerson+ "&cmbYear=" + cmbYear + "&cmbSource=" + cmbSource + "&headertab=" + headerTab, "_blank");
