$(function(){
	...
	$(document.getElementById("formSearch")).submit(function(){
    	...
    	$.ajax({
			...
			data: $(this).serialize(),
			dataType : "json",
			url:'relWorkedRequests/table_json/',
			success: function(data){
				...
				if(data.length == 0)
					...
					$.each(data, function(key, val) {
						$table.append('<tr><td>'+ val.code +'</td><td>'+ val.subject +'</td><td>'+ val.name +'</td><td>'+ val.date +'</td><td>'+ val.status +'</td></tr>');
	               });	
				}
			},
			beforeSend: function(){
	...
	$(document.getElementById('modalSalvar')).find('form').submit(function(){
        ...
        window.open(path + "/app/reports/relWorkedRequest.php?outputtype=" + outputtype + "&delimiter=" + txtSeparator + "&cmbPerson=" +cmbPerson+ "&status=" +status+ "&fromdate=" + fromdate + "&todate=" + todate + "&headertab=" + headerTab, "_blank");    	
    	objModal.closeModal();
    	return false;
    });
