$(function(){
	...
	$(document.getElementById("formSearch")).submit(function(){
    	...
    	$.ajax({
			...
			data: $(this).serialize(),
			dataType : "json",
			url:'relOpeAverCloseTime/table_json/',
	...
	$(document.getElementById('modalSalvar')).find('form').submit(function(){
        ...
        window.open(path + "/app/reports/relOperatorClosureAttendanceTime.php?outputtype=" + outputtype + "&delimiter=" + txtSeparator + "&operator=" +operator+ "&fromdate=" + fromdate + "&todate=" + todate + "&headertab=" + headerTab, "_blank");
