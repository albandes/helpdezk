$(function(){
			...
			success: function(data){
				...
				if(data.length == 0)
                    ...
                    $.each(data, function(key, val) {                    		
                        $table.append('<tr><td>'+ val.code +'</td><td>'+ val.subject +'</td><td>'+ val.description +'</td></tr>');
                    });	
                }
			},
			beforeSend: function(){
	...
	$(document.getElementById('modalSalvar')).find('form').submit(function(){
        ...
        window.open(path + "/app/reports/relReject.php?outputtype=" + outputtype + "&delimiter=" + txtSeparator + "&cmbCompany=" +cmbCompany+ "&cmbPerson=" +cmbPerson+ "&fromdate=" + fromdate + "&todate=" + todate + "&headertab=" + headerTab, "_blank");
