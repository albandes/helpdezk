$(function(){
	...
	$(document.getElementById("formSearch")).submit(function(){
    	...
    	$.ajax({
			...
			data: $(this).serialize(),
			dataType : "json",
			url:'relSumDepartment/table_json/',
			success: function(data){
				if(data.length == 0)
                    ...
                    $.each(data, function(key, val) {
                        ...
                        $table.append('<tr><td>'+ val.company +'</td><td>'+ val.department +'</td><td align="right">'+ val.att_way +'</td><td class="valor">'+ val.total +'</td><td class="valor">'+ val.min_normal +'</td><td class="valor">'+ val.min_extras +'</td><td class="valor">'+ val.minutes +'</td></tr>');
                    });	
                }
			},
			beforeSend: function(){
	...
	$(document.getElementById('modalSalvar')).find('form').submit(function(){
        ...
        window.open(path + "/app/reports/relSumDepartment.php?outputtype=" + outputtype + "&delimiter=" + txtSeparator + "&fromdate=" + fromdate + "&todate=" + todate + "&cmbcompany=" + cmbcompany + "&headertab=" + headerTab, "_blank");
