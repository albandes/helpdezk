 ...
 *   Variables 
 */
$title 		= html_entity_decode($a_lang['pgr_request_status']);
$page 		= utf8_decode($a_lang['PDF_Page']);
$LineCabec  = array( html_entity_decode($a_lang['status']),  html_entity_decode($a_lang['Requests']));
...
if($_GET['cmbCompany'])
	$where .= " and sol.idperson_juridical = ".$_GET['cmbCompany'];

if($_GET['cmbPerson'])
	$where .= " AND sol.idperson_creator = ".$_GET['cmbPerson'];

$sql = 	"
		select stat.user_view as status, count(sol.code_request) as qtd 
		from hdk_tbstatus stat, hdk_tbrequest sol, hdk_tbrequest_in_charge solgrup 
		where sol.idstatus = stat.idstatus 
		and sol.code_request = solgrup.code_request 
		AND ind_in_charge = 1 
		$date_interval
		$where
		group by stat.idstatus_source, stat.user_view,stat.idstatus order by stat.idstatus_source,stat.idstatus
		";
...
switch ($_GET['outputtype']) {
    ...
    case "CSV":
       	$csv = array();
        while (!$rs->EOF) {
            array_push($csv, $rs->fields['status'] . ',' . $rs->fields['qtd'] . ',' . utf8_decode($motivo));
            $rs->MoveNext();
        } 
    ...
    case "PDF":
        ...
        while (!$rs->EOF) {
        	...
        	$pdf->Row(array($rs->fields['status'],$rs->fields['qtd']));
            $linha++;
            $rs->MoveNext();
        }
