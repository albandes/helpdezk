...
$title 		= utf8_decode($a_lang['PDF_request_report'])." ".$period;
...
if($_GET['fromdate'] || $_GET['todate']){
	...
	$date_interval = $pipe->mysql_date_condition($date_field, $_POST['fromdate'] , $_POST['todate'], $lang_default) ;
...
switch ($_GET['outputtype']) {
    ...
    case "CSV":
       	$csv = array();
        while (!$rs->EOF) {
            array_push($csv, $rs->fields['code_request'] . ',' . utf8_decode($rs->fields['name']) . ',' . utf8_decode($rs->fields['company']) . ',' . utf8_decode($rs->fields['subject']) . ',' . utf8_decode($rs->fields['date']). ',' . utf8_decode($rs->fields['priority']). ',' . utf8_decode($rs->fields['status'])) ;
            $rs->MoveNext();
        } 
    ...
    case "XLS":
        ...
        while (!$rs->EOF) {
            ...
            $excel[$i] = array  (
                                ...
                                2 => utf8_decode($rs->fields['company']),
                                3 => utf8_decode($rs->fields['subject']), 
                                4 => utf8_decode($rs->fields['date']),
                                5 => utf8_decode($rs->fields['priority']),
    ...
    case "PDF":
        ...
        class PDF extends FPDF {
            ...
            function Header() {
				...
				$this->Cell($leftmargin);
				$this->Cell(0,5,date("d/m/Y"),0,0,'C');				
                $this->Ln(8);
                $this->cabec();
            }
        ...
        while (!$rs->EOF) {
        	...
        	$pdf->Row(array($rs->fields['code_request'],utf8_decode($rs->fields['name']),utf8_decode($rs->fields['company']), utf8_decode($rs->fields['subject']), utf8_decode($rs->fields['date']), utf8_decode($rs->fields['priority']), utf8_decode($rs->fields['status'])));
            $linha++;
            $rs->MoveNext();
        }
