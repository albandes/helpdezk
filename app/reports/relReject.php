 ...
 *   Variables 
 */
$title 		= utf8_decode($a_lang['PDF_reject']);
$page 		= utf8_decode($a_lang['PDF_Page']);
$LineCabec  = array( html_entity_decode($a_lang['Code']),  html_entity_decode($a_lang['Subject']),  html_entity_decode($a_lang['Reason']));
...
$sql = 	"
		SELECT sol.code_request, sol.subject, apont.description 
		FROM hdk_tbrequest sol, hdk_tbnote apont 
		WHERE sol.code_request = apont.code_request AND sol.idstatus = 6 AND apont.idtype = 3 
		AND (apont.description LIKE '%Solicita&ccedil;&atilde;o n&atilde;o pode ser atendida:%' 
		OR apont.description LIKE '%Request Could not be attended:%') 
		$date_interval
		$where
		";
...
switch ($_GET['outputtype']) {
    ...
    case "CSV":
        ...
        while (!$rs->EOF) {
            ...
            array_push($csv, $rs->fields['code_request'] . ',' . $rs->fields['subject'] . ',' . utf8_decode($motivo));
            $rs->MoveNext();
        } 
    ...
    case "XLS":
        ...
        while (!$rs->EOF) {
            ...
            $excel[$i] = array  (
                                ...
                                1 => utf8_decode($rs->fields['subject']),
                                2 => utf8_decode($motivo)
                                );
    ...
    case "PDF":
        ...
        class PDF extends FPDF {			
            ...
            function cabec() {
                ...
                $this->Cell(70, 4, $LineCabec[1], 0, 0, 'L', 1);
                $this->Cell(75, 4, $LineCabec[2], 0, 0, 'L', 1);                
                $this->Ln(5);
            }
        ...
        while (!$rs->EOF) {
        	...
        	$pdf->Row(array($rs->fields['code_request'],$rs->fields['subject'],utf8_decode($motivo)));
            $linha++;
            $rs->MoveNext();
        }
