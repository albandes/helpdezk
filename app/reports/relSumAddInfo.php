 ...
 *   Variables 
 */
$title 		= html_entity_decode($a_lang['pgr_sum_add_info'],ENT_COMPAT, 'ISO-8859-1');
...
$sql = "SELECT COUNT(a.idaddinfo) AS total, b.name
...
switch ($_GET['outputtype']) {
    ...
    case "PDF":
        require_once('../../includes/classes/fpdf/' . 'fpdf.php');
         ...
         class PDF extends FPDF {
            ...
            function cabec() {
                ...
                $this->Cell($leftmargin);
                $this->Cell(100, 5, $LineCabec[0], 0, 0, 'L', 1);
                $this->Cell(50, 5, $LineCabec[1], 0, 0, 'R', 1);
                $this->Ln(5);
            }
        ...
        while (!$rs->EOF) {
			...
			$pdf->Row(array(utf8_decode($rs->fields['name']),utf8_decode($rs->fields['total'])));
			$linha++;
			$total += $rs->fields['total'];
            $rs->MoveNext();
        }
