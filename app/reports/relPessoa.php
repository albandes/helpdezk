 ...
 *   Variables 
 */
$title      = utf8_decode($a_lang['PDF_person_report']);
$page		= utf8_decode($a_lang['PDF_Page']);
$LineCabec  = array($a_lang['login'], $a_lang['Name'], $a_lang['Type'], $a_lang['Company']); 
/* ----------------------------------------------------------------------------------------*/
...
if ($id != "ALL"){
    $SQL .= "and b.idtypeperson = $id " ;
}

$SQL .= "order by a.login ASC";

$rs = $db->Execute($SQL);
if(!$rs) die("Erro : " . $db->ErrorMsg());
...
switch ($_GET['outputtype']) {
    ...
    case "CSV":
        $csv = array();
        while (!$rs->EOF) {
        	if(!$rs->fields['login']) $login = "-";
			else $login = $rs->fields['login'];
			if(!$rs->fields['company']) $company = "-";
			else $company = $rs->fields['company'];
				
            array_push($csv, $login . ',' . utf8_decode($rs->fields['name']) . ',' . utf8_decode($rs->fields['typeperson']). ',' . utf8_decode($company)) ;
            $rs->MoveNext();
        } 
    ...
    case "XLS":
        ...
        while (!$rs->EOF) {
            ...
            $excel[$i] = array  (
                                ...
                                2 => utf8_decode($rs->fields['typeperson']),
    ...
    case "PDF":
        ...
        class PDF extends FPDF
        {
            ...
            function Header()
            {
                ...
                //Title
                $this->Cell(170,0,$title,0,0,'C');                
                //Line break
                $this->Ln(5);
				$this->SetFont('Arial','B',8);
				$this->Cell(170,0,date("d/m/Y"),0,0,'C');
				$this->Ln(12);
                $this->cabec();
            }
            function Footer()
            ...
            function cabec()
            {
                ...
                $this->Cell(65,4,$LineCabec[1],0,0,'L',1);
                $this->Cell(30,4,$LineCabec[2],0,0,'L',1);
				$this->Cell(64,4,$LineCabec[3],0,0,'L',1);
                $this->Ln(5);
            }
        }
        ...
        $pdf->AddPage();
		$pdf->SetFont('Arial','',8);
		$pdf->SetWidths(array(30,65,30,64));
		$linha = 0;
        while (!$rs->EOF) {			
			$pdf->Cell($leftmargin);
        	...
        	$pdf->Row(array($rs->fields['login'], utf8_decode($rs->fields['name']), utf8_decode($rs->fields['typeperson']),utf8_decode($rs->fields['company'])));
            $linha++;
            $rs->MoveNext();
        }        
