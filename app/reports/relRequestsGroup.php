 ...
 *   Variables 
 */
$title 		= html_entity_decode($a_lang['pgr_request_group']);
    ...
    case "CSV":
		...
		foreach ($output as $key => $val) {			
			...
			foreach ($val['sol'] as $key => $sol) {				
				array_push($csv, utf8_decode($sol['code']).','.utf8_decode($sol['name']).','.utf8_decode($sol['company']).','.utf8_decode($sol['subject']).','.utf8_decode($sol['entry_date']).','.utf8_decode($sol['priority']).','.utf8_decode($sol['status']));
				$total++;
			}			
			if($val['total']['new']){				
