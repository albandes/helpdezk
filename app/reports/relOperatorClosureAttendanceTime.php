...
$title 		= html_entity_decode($a_lang['Request_Ope_Avg_Clos_Time'], ENT_COMPAT, 'ISO-8859-1')." ".$period;;
$page 		= utf8_decode($a_lang['PDF_Page']);
$LineCabec  = array( html_entity_decode($a_lang['Operator']), html_entity_decode($a_lang['Company']),  html_entity_decode($a_lang['Minimum'], ENT_COMPAT, 'ISO-8859-1'),  html_entity_decode($a_lang['Maximum'], ENT_COMPAT, 'ISO-8859-1'),  html_entity_decode($a_lang['Middle'], ENT_COMPAT, 'ISO-8859-1'));
...
switch ($_GET['outputtype']) {
    ...
    case "CSV":
       	...
       	$i = 0;
        while (!$rs->EOF) {
            array_push($csv, $rs->fields['name'] . ',' . $rs->fields['company'] . ',' . $pipe->conv_minute_hour($rs->fields['min_time']) . ',' . $pipe->conv_minute_hour($rs->fields['max_time']) . ',' . $pipe->conv_minute_hour($rs->fields['avg_time'])) ;
        ...
        array_push($csv, html_entity_decode($a_lang['Average_time'], ENT_COMPAT, 'ISO-8859-1') . ', ,'. $pipe->conv_minute_hour($min/$i) . ',' . $pipe->conv_minute_hour($max/$i) . ',' . $pipe->conv_minute_hour($avg/$i) );
    ...
    case "XLS":
        ...
        while (!$rs->EOF) {
            ...
            $excel[$i] = array  (
                                ...
                                2 => $pipe->conv_minute_hour($rs->fields['min_time']), 
                                3 => $pipe->conv_minute_hour($rs->fields['max_time']),
                                4 => $pipe->conv_minute_hour($rs->fields['avg_time'])
                                );
            $i++;
			$min += $rs->fields['min_time'];
			$max += $rs->fields['max_time'];
		...
		$excel[$i] = array  (
                                0 => html_entity_decode($a_lang['Average_time'], ENT_COMPAT, 'ISO-8859-1'),
                                1 => "",
                                2 => $pipe->conv_minute_hour($min/$i), 
                                3 => $pipe->conv_minute_hour($max/$i),
                                4 => $pipe->conv_minute_hour($avg/$i)
                                );
    ...
    case "PDF":
		...
		$pdf->SetWidths(array(106,27,27,27));
		$pdf->Cell($leftmargin);
		if(($linha % 2) == 0) $pdf->SetFillColor(255,255,255);
		else $pdf->SetFillColor(230,230,250);
		$pdf->SetFont('Arial','B',8);
		$pdf->SetAligns(array("L","L","L","L"));
		$pdf->Row(array(html_entity_decode($a_lang['Average_time'], ENT_COMPAT, 'ISO-8859-1'), $pipe->conv_minute_hour($min/$i), $pipe->conv_minute_hour($max/$i), $pipe->conv_minute_hour($avg/$i)));
