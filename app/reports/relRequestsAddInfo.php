 ...
 *   Variables 
 */
$title 		= html_entity_decode($a_lang['pgr_req_add_info'],ENT_COMPAT, 'ISO-8859-1');
...
$LineCabec  = array(
					...
					html_entity_decode($a_lang['Additional_information'],ENT_COMPAT, 'ISO-8859-1')
				);
...
switch ($_GET['outputtype']) {
    ...
    case "CSV":
		...
		while (!$rs->EOF) {
        	...
        	array_push($csv,
        		...
        		utf8_decode($rs->fields['person']) . ',' .
        		utf8_decode($rs->fields['date']) . ',' .
	            utf8_decode($rs->fields['addinfo'])
			);
    ...
    case "XLS":
		...
		while (!$rs->EOF) {
        	 $excel[$i] = array  (
            					...
            					1 => utf8_decode($rs->fields['person']),
            					2 => utf8_decode($rs->fields['date']),
            					3 => utf8_decode($rs->fields['addinfo'])
                                );								
			$i++;
            $rs->MoveNext();
        }
    ...
    case "PDF":
        require_once('../../includes/classes/fpdf/' . 'fpdf.php');
         ...
         class PDF extends FPDF {
            ...
            function cabec() {
                ...
                $this->Cell(62, 5, $LineCabec[1], 0, 0, 'L', 1);
				$this->Cell(27, 5, $LineCabec[2], 0, 0, 'L', 1);
				$this->Cell(55, 5, $LineCabec[3], 0, 0, 'L', 1);
                $this->Ln(5);
            }
        ...
        $pdf->AddPage();
		$pdf->SetFont('Arial','',8);
		$pdf->SetWidths(array(25,62,27,55));
        ...
        while (!$rs->EOF) {
			...
			$pdf->Row(array(utf8_decode($rs->fields['code_request']),utf8_decode($rs->fields['person']),utf8_decode($rs->fields['date']),utf8_decode($rs->fields['addinfo'])));
			$linha++;
            $rs->MoveNext();
        }
