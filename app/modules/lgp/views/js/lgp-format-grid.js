$(document).ready(function () {
    ...
    var grid = $("#table_list_format");

    grid.jqGrid({
        url: path+"/lgp/lgpFormat/jsonGrid",
        datatype: "json",
        mtype: 'POST',
       ...
       colNames:['',makeSmartyLabel('Name'),makeSmartyLabel('status'),'', ''],
        colModel:[
            {name:'id', index:'idformatocoleta',editable: false, width:9, align:"center",sortable: false, search:false, hidden: true },
        ...
        pager: "#pager_list_format",
        viewrecords: true,
        caption: makeSmartyLabel('pgr_format'),
        hidegrid: false,
        toppager:false,
        //jqModal: false,
        //modal: true,
        ondblClickRow: function(rowId) {
            var formatID = grid.jqGrid('getCell', rowId, 'id');
            location.href = path + "/lgp/lgpFormat/viewFormat/formatID/" + formatID;
        },
    ...
    grid.jqGrid('setCaption', '<strong>'+makeSmartyLabel('pgr_format')+'</strong>');

    // Setup buttons
    grid.navGrid('#pager_list_format',{edit:false,add:false,del:false,search:true,searchtext: makeSmartyLabel('Search'),refreshtext: makeSmartyLabel('Grid_reload'),cloneToTop: true});
    ...
    $("#btnCreate").click(function(){
        location.href = path + "/lgp/lgpFormat/formCreate" ;
    });

    $("#btnUpdate").click(function(){
        var myGrid = $('#table_list_format'),
            selRowId = myGrid.jqGrid ('getGridParam', 'selrow'),
            formatID = myGrid.jqGrid ('getCell', selRowId, 'id');

        if (!formatID) {
            showAlert(makeSmartyLabel('Alert_select_one'),'warning','');
        } else {
            location.href = path + "/lgp/lgpFormat/formUpdate/formatID/" + formatID;
        }
    });
    ...
    $("#btnEnable").click(function(){
        ...
        if($("#btnEnable").hasClass('active')){
            if(!formatID)
                showAlert(makeSmartyLabel('Alert_select_one'),'warning',path + '/lgp/lgpFormat/index');
            else
                postStatus(formatID,'A');
        }
...
function postStatus(formatID,newStatus)
{
    ...
    $.ajax({
        type: "POST",
        url: path + '/lgp/lgpFormat/statusFormat',
        dataType: 'json',
        data: {
            formatID: formatID,
            newstatus: newStatus
