$(document).ready(function () {
    ...
    var grid = $("#table_list_holdertype");

    grid.jqGrid({
        url: path+"/lgp/lgpHolderType/jsonGrid",
        datatype: "json",
        mtype: 'POST',
       ...
       colNames:['',makeSmartyLabel('Name'),makeSmartyLabel('status'),'', ''],
        ...
        pager: "#pager_list_holdertype",
        viewrecords: true,
        caption: makeSmartyLabel('pgr_holdertype'),
        hidegrid: false,
        toppager:false,
        //jqModal: false,
        //modal: true,
        ondblClickRow: function(rowId) {
            var holdertypeID = grid.jqGrid('getCell', rowId, 'id');
            location.href = path + "/lgp/lgpHolderType/viewHoldertype/holdertypeID/" + holdertypeID;
        },
    ...
    grid.jqGrid('setCaption', '<strong>'+makeSmartyLabel('pgr_holdertype')+'</strong>');

    // Setup buttons
    grid.navGrid('#pager_list_holdertype',{edit:false,add:false,del:false,search:true,searchtext: makeSmartyLabel('Search'),refreshtext: makeSmartyLabel('Grid_reload'),cloneToTop: true});
    ...
    $("#btnCreate").click(function(){
        location.href = path + "/lgp/lgpHolderType/formCreate" ;
    });

    $("#btnUpdate").click(function(){
        var myGrid = $('#table_list_holdertype'),
            selRowId = myGrid.jqGrid ('getGridParam', 'selrow'),
            holdertypeID = myGrid.jqGrid ('getCell', selRowId, 'id');

        if (!holdertypeID) {
            showAlert(makeSmartyLabel('Alert_select_one'),'warning','');
        } else {
            location.href = path + "/lgp/lgpHolderType/formUpdate/holdertypeID/" + holdertypeID;
        }
    });
    ...
    $("#btnEnable").click(function(){
        ...
        if($("#btnEnable").hasClass('active')){
            if(!holdertypeID)
                showAlert(makeSmartyLabel('Alert_select_one'),'warning',path + '/lgp/lgpHolderType/index');
            else
                postStatus(holdertypeID,'A');
        }
...
function postStatus(holdertypeID,newStatus)
{
    ...
    $.ajax({
        type: "POST",
        url: path + '/lgp/lgpHolderType/statusHoldertype',
        dataType: 'json',
        data: {
