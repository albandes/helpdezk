$(document).ready(function () {
    ...
    var grid = $("#table_list_mode");

    grid.jqGrid({
        url: path+"/lgp/lgpMode/jsonGrid",
        datatype: "json",
        mtype: 'POST',
       ...
       colNames:['',makeSmartyLabel('Name'),makeSmartyLabel('status'),'', ''],
        colModel:[
            {name:'id', index:'idformacoleta',editable: false, width:9, align:"center",sortable: false, search:false, hidden: true },
        ...
        pager: "#pager_list_mode",
        viewrecords: true,
        caption: makeSmartyLabel('pgr_mode'),
        hidegrid: false,
        toppager:false,
        //jqModal: false,
        //modal: true,
        ondblClickRow: function(rowId) {
            var modeID = grid.jqGrid('getCell', rowId, 'id');
            location.href = path + "/lgp/lgpMode/viewMode/modeID/" + modeID;
        },
    ...
    grid.jqGrid('setCaption', '<strong>'+makeSmartyLabel('pgr_mode')+'</strong>');

    // Setup buttons
    grid.navGrid('#pager_list_mode',{edit:false,add:false,del:false,search:true,searchtext: makeSmartyLabel('Search'),refreshtext: makeSmartyLabel('Grid_reload'),cloneToTop: true});
    ...
    $("#btnCreate").click(function(){
        location.href = path + "/lgp/lgpMode/formCreate" ;
    });

    $("#btnUpdate").click(function(){
        var myGrid = $('#table_list_mode'),
            selRowId = myGrid.jqGrid ('getGridParam', 'selrow'),
            modeID = myGrid.jqGrid ('getCell', selRowId, 'id');

        if (!modeID) {
            showAlert(makeSmartyLabel('Alert_select_one'),'warning','');
        } else {
            location.href = path + "/lgp/lgpMode/formUpdate/modeID/" + modeID;
        }
    });
    ...
    $("#btnEnable").click(function(){
        ...
        if($("#btnEnable").hasClass('active')){
            if(!modeID)
                showAlert(makeSmartyLabel('Alert_select_one'),'warning',path + '/lgp/lgpMode/index');
            else
                postStatus(modeID,'A');
        }
...
function postStatus(modeID,newStatus)
{
    ...
    $.ajax({
        type: "POST",
        url: path + '/lgp/lgpMode/statusMode',
        dataType: 'json',
        data: {
            modeID: modeID,
            newstatus: newStatus
...
function fontColorMode(cellvalue, options, rowObject) {
    var color = "blue";
