$(document).ready(function () {
    ...
    var grid = $("#table_list_type");

    grid.jqGrid({
        url: path+"/lgp/lgpType/jsonGrid",
        datatype: "json",
        mtype: 'POST',
       ...
       colNames:['',makeSmartyLabel('Name'),makeSmartyLabel('status'),'', ''],
        ...
        pager: "#pager_list_type",
        viewrecords: true,
        caption: makeSmartyLabel('pgr_type'),
        hidegrid: false,
        toppager:false,
        //jqModal: false,
        //modal: true,
        ondblClickRow: function(rowId) {
            var tipoID = grid.jqGrid('getCell', rowId, 'id');
            location.href = path + "/lgp/lgpType/viewType/tipoID/" + tipoID;
        },
    ...
    grid.jqGrid('setCaption', '<strong>'+makeSmartyLabel('pgr_type')+'</strong>');

    // Setup buttons
    grid.navGrid('#pager_list_type',{edit:false,add:false,del:false,search:true,searchtext: makeSmartyLabel('Search'),refreshtext: makeSmartyLabel('Grid_reload'),cloneToTop: true});
    ...
    $("#btnCreate").click(function(){
        location.href = path + "/lgp/lgpType/formCreate" ;
    });

    $("#btnUpdate").click(function(){
        var myGrid = $('#table_list_type'),
            selRowId = myGrid.jqGrid ('getGridParam', 'selrow'),
            tipoID = myGrid.jqGrid ('getCell', selRowId, 'id');

        if (!tipoID) {
            showAlert(makeSmartyLabel('Alert_select_one'),'warning','');
        } else {
            location.href = path + "/lgp/lgpType/formUpdate/tipoID/" + tipoID;
        }
    });
    ...
    $("#btnEnable").click(function(){
        ...
        if($("#btnEnable").hasClass('active')){
            if(!tipoID)
                showAlert(makeSmartyLabel('Alert_select_one'),'warning',path + '/lgp/lgpType/index');
            else
                postStatus(tipoID,'A');
        }
...
function postStatus(tipoID,newStatus)
{
    ...
    $.ajax({
        type: "POST",
        url: path + '/lgp/lgpType/statusType',
        dataType: 'json',
        data: {
            tipoID: tipoID,
            newstatus: newStatus
