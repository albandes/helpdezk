$(document).ready(function () {
    ...
    var grid = $("#table_list_legalGround");

    grid.jqGrid({
        url: path+"/lgp/lgpLegalGround/jsonGrid",
        datatype: "json",
        mtype: 'POST',
       ...
       colNames:['',makeSmartyLabel('Name'),makeSmartyLabel('status'),'', ''],
        colModel:[
            {name:'id', index:'baselegal',editable: false, width:9, align:"center",sortable: false, search:false, hidden: true },
        ...
        pager: "#pager_list_legalGround",
        viewrecords: true,
        caption: makeSmartyLabel('pgr_legalground'),
        hidegrid: false,
        toppager:false,
        //jqModal: false,
        //modal: true,
        ondblClickRow: function(rowId) {
            var legalGroundID = grid.jqGrid('getCell', rowId, 'id');
            location.href = path + "/lgp/lgpLegalGround/viewLegalGround/legalGroundID/" + legalGroundID;
        },
    ...
    grid.jqGrid('setCaption', '<strong>'+makeSmartyLabel('pgr_legalground')+'</strong>');

    // Setup buttons
    grid.navGrid('#pager_list_legalGround',{edit:false,add:false,del:false,search:true,searchtext: makeSmartyLabel('Search'),refreshtext: makeSmartyLabel('Grid_reload'),cloneToTop: true});
    ...
    $("#btnCreate").click(function(){
        location.href = path + "/lgp/lgpLegalGround/formCreate" ;
    });

    $("#btnUpdate").click(function(){
        var myGrid = $('#table_list_legalGround'),
            selRowId = myGrid.jqGrid ('getGridParam', 'selrow'),
            legalGroundID = myGrid.jqGrid ('getCell', selRowId, 'id');

        if (!legalGroundID) {
            showAlert(makeSmartyLabel('Alert_select_one'),'warning','');
        } else {
            location.href = path + "/lgp/lgpLegalGround/formUpdate/legalGroundID/" + legalGroundID;
        }
    });
    ...
    $("#btnEnable").click(function(){
        ...
        if($("#btnEnable").hasClass('active')){
            if(!legalGroundID)
                showAlert(makeSmartyLabel('Alert_select_one'),'warning',path + '/lgp/lgpLegalGround/index');
            else
                postStatus(legalGroundID,'A');
        }
...
function postStatus(legalGroundID,newStatus)
{
    ...
    $.ajax({
        type: "POST",
        url: path + '/lgp/lgpLegalGround/statusLegalGround',
        dataType: 'json',
        data: {
