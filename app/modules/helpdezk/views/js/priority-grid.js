$(document).ready(function () {
   ...
   $('#filter-list').change(function(){
     ...
     * */ 
    var colM = [ 
        { title: vocab["ID"], width: '10%', dataIndx: "idpriority", hidden:true, halign: "center"  },        
        { title: "<b>"+vocab["Priority"]+"</b> ", width: '55%', dataIndx: "name", halign: "center"  },
        { title: "<b>"+vocab["Exhibition_order"]+"</b> ", width: '15%', dataIndx: "order", align: "center", halign: "center"  },        
        ...
        { title: "<b>"+vocab["Default"]+"</b> ", width: '9%', dataIndx: "default", align: "center", halign: "center"  },
    ...
    var dataModel = {
        ...
        dataType: "JSON",
        method: "POST",
        url: path + '/helpdezk/hdkPriority/jsonGrid',
    ...
    var obj = { 
        ...
        selectChange: function (evt, ui) {
            var rowIndx = getRowIndx('grid_priorities'),
                row = $("#grid_priorities").pqGrid('getRowData', {rowIndx: rowIndx}),
        ...
        rowDblClick: function (evt, ui) {
            ...
            location.href = path + "/helpdezk/hdkPriority/formUpdate/"+row.idpriority;
        }
    };

    $("#grid_priorities").pqGrid(obj);

    //Grid i18n
    var locale = default_lang.replace('_','-');
    $("#grid_priorities").pqGrid("option", $.paramquery.pqGrid.regional[locale]);
    $("#grid_priorities").find(".pq-pager").pqPager("option", $.paramquery.pqPager.regional[locale]);
    ...
    $("#btnCreate").click(function(){
        location.href = path + "/helpdezk/hdkPriority/formCreate";
    });

    $("#btnUpdate").click(function(){
        var rowIndx = getRowIndx('grid_priorities'),msg="";
        
        if (rowIndx != null) {
            var row = $("#grid_priorities").pqGrid('getRowData', {rowIndx: rowIndx});
            location.href = path + "/helpdezk/hdkPriority/formUpdate/"+row.idpriority;
    ...
    $("#btnEnable").click(function(){
        if(!$("#btnEnable").hasClass('disabled')){
            ...
            if (rowIndx != null) {
                var row = $("#grid_priorities").pqGrid('getRowData', {rowIndx: rowIndx});
                postStatus(row.idpriority,'A',row.default_val);
            }else{
                msg = vocab['Alert_select_one'];
                showAlert(msg,'warning');
            }
    ...
    $("#btnDisable").click(function(){
        if(!$("#btnDisable").hasClass('enabled')){
            ...
            if (rowIndx != null) {
                var row = $("#grid_priorities").pqGrid('getRowData', {rowIndx: rowIndx});
                postStatus(row.idpriority,'I',row.default_val);
            }else{
                msg = vocab['Alert_select_one'];
                showAlert(msg,'warning');
            }
    ...
    $("#btnDelete").click(function(){
        ...
        if (rowIndx != null) {
            ...
            if(row.idpriority <= 5){
                showAlert(vocab['priority_not_delete'],'warning','');
            }else{
                $.ajax({
                    type: "POST",
                    url: path + '/helpdezk/hdkPriority/checkDelete',
                    dataType: 'json',
                    data: {
                        _token : $("#_token").val(),
                        priorityId: row.idpriority
                    },
                    error: function (ret) {
                    ...
                    success: function(ret){        
                        ...
                        if(obj.success) {
                            $("#modal-delete-priority-id").val(row.idpriority);
                            $("#modal-delete-priority-is-default").val(row.default_val);
                            $("#modal-delete-priority-name").val(row.name);
                            $("#modal-delete-priority-order").val(row.order);
                            $("#modal-delete-priority-color").val(row.color_val);
    ...
    $("#btnDeleteYes").click(function(){
        ...
        if(!$("#btnDeleteYes").hasClass('disabled')){
            $.ajax({
                type: "POST",
                url: path + '/helpdezk/hdkPriority/deletePriority',
                dataType: 'json',
                data: {
                    _token : $("#_token").val(),
                    priorityId: $("#modal-delete-priority-id").val(),
                    isDefault: $("#modal-delete-priority-is-default").val()
                },
                error: function (ret) {
                    modalAlertMultiple('danger',vocab['generic_error_msg'],'alert-delete-priority');
                },
                success: function(ret){
                    ...
                    if(obj.success) {
                        modalAlertMultiple('success',vocab['Alert_deleted'],'alert-delete-priority');
                        setTimeout(function(){
                            $('#modal-priority-delete').modal('hide');
                            $("#grid_priorities").pqGrid("refreshDataAndView");
                        },4000);
                    } else {
                        modalAlertMultiple('danger',obj.message,'alert-delete-priority');
                    }
                },
                beforeSend: function(){
...
/**
 * Change priority's status
 * @param  {int}priorityId 
 * @param  {string} newStatus
 * @return {void}      
 */
function postStatus(priorityId,newStatus,isDefault)
{
    ...
    $.ajax({
        type: "POST",
        url: path + '/helpdezk/hdkPriority/changeStatus',
        dataType: 'json',
        data: {
            _token: $("#_token").val(),
