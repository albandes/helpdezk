$(document).ready(function () {
     ...
     * */ 
    var colM = [
        { title: vocab["ID"], width: '10%', dataIndx: "idconfig", hidden:true, halign: "center"  },        
        { title: "<b>"+vocab["Name"]+"</b> ", width: '90%', dataIndx: "name", align: "left", halign: "center"  },
    ...
    var obj = { 
        ...
        colModel: colM,
        editable: false,
        title: "<b>"+vocab['pgr_erp_emailtemplate']+"</b>",
        ...
        selectChange: function (evt, ui) {
            var rowIndx = getRowIndx('grid_email_config'),
                row = $("#grid_email_config").pqGrid('getRowData', {rowIndx: rowIndx}),
        ...
        rowDblClick: function (evt, ui) {
            ...
            location.href = path + "/helpdezk/hdkEmailConfig/formUpdate/"+row.idconfig;
        }
    };

    $("#grid_email_config").pqGrid(obj);

    //Grid i18n
    var locale = default_lang.replace('_','-');
    $("#grid_email_config").pqGrid("option", $.paramquery.pqGrid.regional[locale]);
    $("#grid_email_config").find(".pq-pager").pqPager("option", $.paramquery.pqPager.regional[locale]);
    ...
    $("#btnModalSearch").click(function(){
        ...
        $("#grid_email_config").pqGrid( "option", "dataModel.postData", function(){
            return {filterIndx:filterIndx,filterValue:filterValue,filterOperation:filterOperation,allRecords:flg};
        } );
        
        $("#grid_email_config").pqGrid("refreshDataAndView");
    });
    ...
    $("#btnCreate").click(function(){
        location.href = path + "/helpdezk/hdkEmailConfig/formCreate";
    });

    $("#btnUpdate").click(function(){
        var rowIndx = getRowIndx('grid_email_config'),msg="";
        
        if (rowIndx != null) {
            var row = $("#grid_email_config").pqGrid('getRowData', {rowIndx: rowIndx});
            location.href = path + "/helpdezk/hdkEmailConfig/formUpdate/"+row.idconfig;
 ...
 * @return {void}      
 */
function postStatus(featureId,newStatus)
{
