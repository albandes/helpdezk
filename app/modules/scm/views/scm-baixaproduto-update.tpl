    ...
    <div id="page-wrapper" class="gray-bg">
        ...
        <div class="wrapper wrapper-content  ">
            <div class="row wrapper white-bg ibox-title">
                <div class="col-sm-4">
                    <h4>{$smarty.config.cat_records} / {$smarty.config.pgr_scm_baixa_produto} / <strong>{$smarty.config.edit}</strong></h4>
                </div>
        ...
        <!-- Form area -->
        <form method="get" class="form-horizontal" id="update-baixaproduto-form">

            <!-- Hidden -->
            <input type="hidden" name="_token" id= "_token" value="{$token}">
            <input type="hidden" name="idbaixa" id= "idbaixa" value="{$idbaixa}">
            ...
            <div class="row wrapper  white-bg ">
                ...
                <div class="col-sm-11 b-l">
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            {if $tipo== "D"}
                                <label class="radio-inline i-checks"><input type="radio" name="tipo" value="D" class="control-label" checked disabled>&nbsp;&nbsp;{$smarty.config.donation}</label>
                                <label class="radio-inline i-checks"><input type="radio" name="tipo" value="DE" class="control-label" disabled>&nbsp;&nbsp;{$smarty.config.discard}</label>
                                
                            {else}
                                <label class="radio-inline i-checks"><input type="radio" name="tipo" value="D" class="control-label" disabled>&nbsp;&nbsp;{$smarty.config.donation}</label>
                                <label class="radio-inline i-checks"><input type="radio" name="tipo" value="DE" class="control-label" checked disabled>&nbsp;&nbsp;{$smarty.config.discard}</label>
                            {/if}
                    ...
                    <div class="col-sm-10 b-l">
                        <table id="itemList" class="table table-hover">
                            ...
                            {foreach $arrItens as $key => $value}
                                ...
                                <tr>
                                    <td class="text-left">
                                        <input type="hidden" id="iditembaixas_{$i}" name="iditembaixas[]" value="{$value.iditembaixa}" readonly>
                                        <select class="form-control input-sm produtos" name="produtos[]" id="produtos_{$i}">
                                            {html_options values=$produtoids output=$produtovals selected=$value.idproduto}
                                        </select>
                                        <input type="hidden" id="numId" value="{$i}"/>
                                    </td>
                                    <td class="text-right">
                                        <input type="number" name="quantidades[]" id="quantidades_{$i}" class="form-control input-sm text-right qtCheck" placeholder="{$plh_quantidade}" step="0.25" data-qtcheck="{$i}" value="{$value.quantidade}" />
                                    </td>
                                    <td class="text-center"><a href="javascript:;" onclick="removeRow(this,'itemList','upd')" class="btn btn-danger bt-xs"><i class="fa fa-times"></i></a></td>
                                </tr>
                            {/foreach}
                            </tbody>
                        </table>
            ...
            <div class="row wrapper  white-bg ">
                <div class="col-sm-12 b-l">
                    <div id="alert-update-baixaproduto"></div>
                </div>
            </div>
                ...
                <div class="col-sm-12 b-l">
                    <div class="form-group">
                        <div class="col-sm-12">
                            ...
                            <button type="button" class="btn btn-primary btn-md " id="btnUpdateBaixa" >
