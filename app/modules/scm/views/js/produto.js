$(document).ready(function () {
    ...
    grid.jqGrid({
        url: path+"/scm/scmProduto/jsonGrid",
        datatype: "json",
        mtype: 'POST',
        ...
        colNames:['','Nome','Estoque Inicial','Estoque','Estoque mínimo',' Status ', ''],
        colModel:[
            ...
            {name:'estoque_inicial',index:'estoque_inicial', editable: true, width:25, search:true, sorttype: 'string',searchoptions: { sopt: ['eq', 'bw', 'bn', 'cn', 'nc', 'ew', 'en']}, hidden: true },
            {name:'estoque_atual',index:'estoque_atual', editable: true, width:25, search:true, sorttype: 'string',searchoptions: { sopt: ['eq', 'bw', 'bn', 'cn', 'nc', 'ew', 'en']}, hidden: false },
            {name:'estoque_minimo',index:'estoque_minimo', editable: true, width:25, search:false, sorttype: 'string',searchoptions: { sopt: ['eq', 'bw', 'bn', 'cn', 'nc', 'ew', 'en']} },
        ...
        ondblClickRow: function(rowId) {
            var idProduto = grid.jqGrid('getCell', rowId, 'id');
            location.href = path + "/scm/scmProduto/echoProduto/idproduto/" + idProduto ;
        },
    ...
    grid.jqGrid('setCaption', 'Produtos');
    ...
    $("#btnCreate").click(function(){
        location.href = path + "/scm/scmProduto/formCreateProduto" ;
    });

    $("#btnUpdate").click(function(){
        var myGrid = $('#table_list_tickets'),
            selRowId = myGrid.jqGrid ('getGridParam', 'selrow'),
            idProduto = myGrid.jqGrid ('getCell', selRowId, 'id');

        if (!idProduto) {
            $("#btn-modal-ok").attr("href", '');
            $('#modal-notification').html('Marque um produto.');
            $("#tipo-alert").attr('class', 'alert alert-danger');
            $('#modal-alert').modal('show');
        } else {
            location.href = path + "/scm/scmProduto/formUpdateProduto/idproduto/" + idProduto ;
        }
    });
