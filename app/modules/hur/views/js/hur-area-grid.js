$(document).ready(function () {
    ...
    var grid = $("#table_list_hurarea");

    grid.jqGrid({
        url: path+"/hur/hurArea/jsonGrid",
        datatype: "json",
        mtype: 'POST',
        sortname: 'description', //initially sorted on code_request
        sortorder: "ASC",
        height: 450,
        autowidth: true,
        shrinkToFit: true,
        ...
        colModel:[
            ...
            {name:'description',index:'description', editable: true, width:150, search:true, sorttype: 'string',searchoptions: { sopt: ['eq', 'bw', 'bn', 'cn', 'nc', 'ew', 'en']} },
        ...
        pager: "#pager_list_hurarea",
        viewrecords: true,
        caption: makeSmartyLabel('pgr_hurarea'),
        hidegrid: false,
        toppager:false,
        //jqModal: false,
        //modal: true,
        ondblClickRow: function(rowId) {
            var areaID = grid.jqGrid('getCell', rowId, 'id');
            location.href = path + "/hur/hurArea/viewArea/areaID/" + areaID;
        },
    ...
    grid.jqGrid('setCaption', '<strong>'+makeSmartyLabel('pgr_hurarea')+'</strong>');

    // Setup buttons
    grid.navGrid('#pager_list_hurarea',{edit:false,add:false,del:false,search:true,searchtext: makeSmartyLabel('Search'),refreshtext: makeSmartyLabel('Grid_reload'),cloneToTop: true});
    ...
    $("#btnCreate").click(function(){
        location.href = path + "/hur/hurArea/formCreate";
    });

    $("#btnUpdate").click(function(){
        var myGrid = $('#table_list_hurarea'),
            selRowId = myGrid.jqGrid ('getGridParam', 'selrow'),
            areaID = myGrid.jqGrid ('getCell', selRowId, 'id');

        if (!areaID) {
            showAlert(makeSmartyLabel('Alert_select_one'),'warning','');
        } else {
            location.href = path + "/hur/hurArea/formUpdate/areaID/" + areaID;
        }
    });
...
function postStatus(areaID,newStatus)
{
    ...
    $.ajax({
        type: "POST",
        url: path + '/hur/hurArea/statusArea',
        dataType: 'json',
        data: {
