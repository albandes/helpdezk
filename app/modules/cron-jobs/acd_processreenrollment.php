...
if($ret['return']){
    $disable = $dbPerson->disableAllEnrollmentRec("WHERE `year` = $year AND record_status = 'A' AND idstatusenrollment = 2");
    if (!$disable){
    ...
    foreach($ret['return'] as $item) {
        ...
        if(!$check){
            $cronSystem->logIt('Get Student ID  - program: '.$this->program.' - method: '. __METHOD__ ,3,'general',__LINE__);
        }

        $retEnrollment = studentEnrollment($check->fields['idstudent'],$item,$dbPerson,$cronSystem);
        if(!$retEnrollment){
            $cronSystem->logIt('Insert Student Enrollment - program: '.$this->program.' - method: '. __METHOD__ ,3,'general',__LINE__);
        }
