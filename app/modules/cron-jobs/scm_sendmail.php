...
// Controllers
if(!setRequire(HELPDEZK_PATH. 'app/modules/scm/controllers/scmCommonController.php',$cronSystem)){exit;}

//Models
if(!setRequire(HELPDEZK_PATH.'app/modules/scm/models/scmemailconfig_model.php',$cronSystem)){exit;} 
...
$moduleID = $cronSystem->getIdModule("Suprimentos");

$scmCommon = new scmCommon();
$dbEmail = new mainemail_model();
$dbFeature = new features_model();
$scmCommon->_serverApi = getScmAPIConfig($dbFeature,$cronSystem);
...
while (!$rsEmails['data']->EOF)
{
    ...
    $ret = $scmCommon->_sendEmail($rsEmails['data']->fields['tag'] , $rsEmails['data']->fields['code']) ;
...
$cronSystem->logIt('Ran By Cron [scm_sendmail.php] - Number of emails to send: ' . $rows ,6,'email');
$cronSystem->logIt('Ran By Cron [scm_sendmail.php] - Sent emails: ' .$i ,6,'email');
...
function getScmAPIConfig($dbFeature,$cronSystem){
    $table = 'scm_tbconfig_category';
    ...
    $get = $dbFeature->getConfigs("scm",$categoriesID);
    if(!$get){
    ...
    if($get->RecordCount() > 0){
        while (!$get->EOF) {
            ...
            if($get->fields['config_name'] == 'api dominio')
