...
while(!$ret->EOF){
    ...
    foreach ($aChilds as $key=>$value){
        ...
        $retStActive = $dbStudent->getTotalStatusActive($value,$currentYear);
        if(!$retStActive){
            $cronSystem->logIt("Can't get child's enrollment status - program: " . __FILE__, 3, 'general', __LINE__);
        }

        if($retStActive && $retStActive->fields['total_active'] > 0) $countSt++;
    }
