...
$where = "AND s.idcurso NOT IN (6,7,8)
...
while(!$ret->EOF){
    ...
    $classHasBil = $dbStudent->classHasBilingual($ret->fields['idturma']);
    if(!$classHasBil){
        $cronSystem->logIt("Data not return - program: " . __FILE__ , 3, 'general', __LINE__);
    }

    if($classHasBil->RecordCount() > 0){
        $bus = array (
            "idIntegracaoTurma"     => $classHasBil->fields['idbilingue'] . $currentYear,
    ...
    $bus = array (
        "idIntegracaoTurma"     => $ret->fields['idturma'] . $currentYear,
        "idIntegracaoAluno"     => $ret->fields['idstudent'],
        "ativo" => $recordSt
    );
